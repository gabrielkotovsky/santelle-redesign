...
console.log("gpt5_context: booting edge function");
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const supabase = createClient(supabaseUrl, serviceKey, {
      ...
      global: { headers: { "X-Client-Info": "gpt5_context/1.0.0" } },
    });
    console.log("[Edge Function] Supabase client created");

    // 0) Check if context analysis already exists in database
    console.log("[Edge Function] Checking for existing context analysis...");
    const { data: existingLog, error: checkError } = await supabase
      .from("test_logs")
      .select("gpt_5_context")
    ...
    if (existingLog?.gpt_5_context) {
      let cachedAnalysis = existingLog.gpt_5_context;
      ...
      if (cachedAnalysis && 
          ...
          Array.isArray(cachedAnalysis.insights)) {
        console.log("[Edge Function] ✅ Valid cached context analysis found, returning it");
        console.log("[Edge Function] Cached insights count:", cachedAnalysis.insights?.length);
      ...
      console.log("[Edge Function] No cached context analysis found, will generate new one");
    }
    ...
    console.log("[Edge Function] Data fetched:", {
      ...
      contextCount: dataUsed.context.length,
      manufacturerTextLength: dataUsed.manufacturer_text.length
    });

    // 2) Call GPT-5 with strict JSON schema for contextual analysis
    console.log("[Edge Function] Calling GPT for contextual analysis...");
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/gpt5_context' \
